...
def test_update_lead_funnel(client, campaign_id):
    ...
    response = client.patch(f"/leads/{lead_id}/funnel", json={"status": "contatado"})
    assert response.status_code == 200
...
def test_update_lead_funnel_to_convertido(client, campaign_id):
    ...
    response = client.patch(f"/leads/{lead_id}/funnel", json={"status": "convertido"})
    assert response.status_code == 200
...
def test_update_lead_funnel_invalid_status(client, campaign_id):
    ...
    response = client.patch(f"/leads/{lead_id}/funnel", json={"status": "status_inexistente"})
    assert response.status_code == 422


def test_update_lead_funnel_not_found(client):
    response = client.patch(f"/leads/{uuid.uuid4()}/funnel", json={"status": "qualificado"})
    assert response.status_code == 404
    assert response.json()["detail"] == "Lead não encontrado"
